...
public final class McpToolsPanel extends Panel {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        UiComponents.drawOptionToggle(
                ...
                smoothToggle("mcptools.chatFb", s.moduleChatFeedback, delta));

        int y4 = y3 + TOGGLE_H + UiTokens.UI_GAP;
        PanelOverlayActions.drawOpenOverlayButton(tr, context, rx, y4, inner, "Open MCPTools overlay");
        int hintY = y4 + PanelOverlayActions.BTN_H + UiTokens.UI_GAP;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (clickToggle(mouseX, mouseY, rx, body, inner)) {
            ...
            if (!s.enabled) {
                ...
                manager.stopRun("MCPTools disabled.");
            }
            manager.saveSettings();
            manager.moduleFeedbackConfigToggle("MCPTools " + (s.enabled ? "enabled" : "disabled"));
            return true;
        }
        if (clickToggle(mouseX, mouseY, rx, body + TOGGLE_H + UiTokens.UI_GAP, inner)) {
            McpToolsOverlay.INSTANCE.toggleOverlayVisible();
            manager.moduleFeedbackConfigToggle("MCPTools overlay " + (s.overlayVisible ? "shown" : "hidden"));
            return true;
        }
        ...
        if (PanelOverlayActions.clickOpenOverlay(mouseX, mouseY, rx, y4, inner)) {
            PanelOverlayActions.openOverlay(McpToolsOverlay.INSTANCE);
            return true;
        }
        return false;
    }
